...
\begin{figure}[H]
    \centering
    \includegraphics[width=13cm]{High_Level_Project_Overview.png}
...
\subsubsection{Start Finish Detector} \label{sec:Start Finish Detector} \todo{mit welchem Theorieteil hat es einen Zusammenhang?}
...
If all points of the planned path are published like the Exploration Algorithm creates them, there would be an issue regarding the Autopilot module of the autonomous system, which controls the car itself. The Autopilot currently drives from point to point as it receives them by the Path Planner. \cite{zur_autopilot}
...
In the end, the most optimal race trajectory line will be outputted by the algorithm. \todo{Reference to external repo}
\begin{figure}[H]
    \centering

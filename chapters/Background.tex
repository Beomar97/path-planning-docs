...
A human follows a path if it goes from point A to point B while following certain signalization, e.g. on hiking trails. It also follows a path if it follows one predetermined for them by a GPS. The difference lies between a manual plan and a machine-generated plan. This section addresses the latter one.
